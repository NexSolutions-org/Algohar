            ...
            <div style="background-color: #FFFFFF; border: 2px solid #E60000; padding: 20px; margin: 20px 0; border-radius: 8px;">
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <tr>
                        ...
                        <td style="padding: 8px 0; color: #000000;">{{ ucfirst($oldStatus) }}</td>
                    </tr>
                    <tr>
                        <td style="padding: 8px 0; font-weight: bold; color: #333333;">Current Status:</td>
                        <td style="padding: 8px 0;">
                            <span style="background-color: {{ $newStatus === 'completed' ? '#E8F5E9' : ($newStatus === 'failed' ? '#FFEBEE' : '#FFF3F3') }}; color: {{ $newStatus === 'completed' ? '#00AA00' : ($newStatus === 'failed' ? '#D32F2F' : '#E60000') }}; border: 1px solid {{ $newStatus === 'completed' ? '#00AA00' : ($newStatus === 'failed' ? '#D32F2F' : '#E60000') }}; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: bold;">
                                {{ ucfirst($newStatus) }}
                            </span>
                        </td>
                    </tr>
